...
async function getDataFromUrl(URL) {
    try {
        ...
        if (!getResponce.ok) {
            throw new Error('Error GET responce')
        }

        const result = await getResponce.json()
        console.log(result)
    } catch (e) {
        console.log(e)
    }
...
async function postDataWithAuth(url, data, authToken) {
    try {
        ...
        console.log("Відповідь на POST-запит:", responseData);
    } catch (e) {
        console.error(e);
    }
}
...
async function displayData() {
    ...
    for (let i = 0; i < 3; i++) {
        ...
        console.log("Поточні дані:", result.value);
    }
}
